...
public class PrefabEditor
{
    ...
    private static void ModifyUITexture()
    {
        ...
        //foreach (string strPreFile in arrPreFiles)
        {
            ...
            string strPreFile = "Assets/Prefabs/UI/mainui/UIHeroHead.prefab";
	...
	{
        string[] arrFiles = Directory.GetFiles("Assets/Textures/npc/", "*.tpsheet", SearchOption.AllDirectories);
		int nIndex = 1;
		foreach(string strFile in arrFiles)
		{
			// 判断是否是图集
			string strTextureFile = strFile.Substring(0, strFile.LastIndexOf(".")) + ".png";
            TextureImporter txImporter = TextureImporter.GetAtPath(strTextureFile) as TextureImporter;
			if (txImporter == null)
			{
				continue;
			}
            Debug.Log(strTextureFile);
            UpdateProgress(nIndex++, arrFiles.Length, "tex : " + strTextureFile);
