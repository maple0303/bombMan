...
//#endif
        ...
        void Awake()
        {
            DragonBonesData dragonBonesData =  LoadData(dragonBonesJSON, textureAtlasJSON);

            if (!string.IsNullOrEmpty(armatureName))
            {
                UnityFactory.factory.BuildArmatureComponent(armatureName, dragonBonesData.name, null, textureAtlasName, gameObject);
            }
